...
export async function POST(req: NextRequest) {
  try {
    ...
    // Check if a bookmark already exists for this story by the current user
    ...
    if (!existingBookmark) {
      ...
      // Increase the bookmarkAmount by 1
      await prisma.story.update({
        where: { id: storyId },
        data: { bookmarkAmount: { increment: 1 } },
      });
      ...
      // Decrease the bookmarkAmount by 1
      await prisma.story.update({
        where: { id: storyId },
        data: { bookmarkAmount: { decrement: 1 } },
      });
